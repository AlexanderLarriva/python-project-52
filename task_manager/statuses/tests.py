...
class StatusesUrlsTest(TestCase):
    ...
    def test_update_page_without_login(self):
        response = self.client.get(reverse('statuses:update_status',
                                           kwargs={'pk': 1}))
        self.assertEqual(response.status_code, 302)
        self.assertTrue(response.url.startswith('/login/'))

    def test_delete_page_without_login(self):
        response = self.client.get(reverse('statuses:delete_status',
                                           kwargs={'pk': 1}))
        self.assertEqual(response.status_code, 302)
        self.assertTrue(response.url.startswith('/login/'))
...
class StatusesTest(TestCase):
    ...
    def test_create_status(self):
        ...
        self.assertEqual(response.status_code, 200)
        # Create status
        response = self.client.post(reverse('statuses:create_status'),
                                    self.new_data)
    ...
    def test_delete_status(self):
        ...
        self.assertEqual(response.status_code, 200)
        # DElete status
        response = self.client.post(reverse('statuses:delete_status',
                                            kwargs={'pk': 3}))
